"""Temporal activities for math evaluation workflow."""
...
class MathEvaluationActivities:
    """Activities for math evaluation workflow."""

    @staticmethod
    async def download_problem_images(question_image_url: str, working_note_url: str) -> Tuple[str, str]:
        """Download both question and working note images from Azure Blob Storage."""
        print(f"📥 Downloading both images from Azure Blob Storage")
        
        try:
            # Use the Azure storage manager to download both images
            question_image_path, working_note_path = await azure_storage.download_both_images(
                question_image_url, working_note_url
            )
    ...
    async def crop_working_note_image(working_note_path: str, bounding_box: BoundingBox) -> str:
        ...
        try:
            ...
            print(f"Failed to crop working note image: {e}")
            raise
    ...
    async def preprocess_images(question_image_path: str, working_note_path: str) -> Tuple[str, str]:
        ...
        try:
            ...
            processed_question_path = await MathEvaluationActivities._preprocess_single_image(
                question_image_path, "question"
            )
            
            # Process working note image
            processed_working_note_path = await MathEvaluationActivities._preprocess_single_image(
                working_note_path, "working_note"
            )
    ...
    async def _preprocess_single_image(image_path: str, image_type: str) -> str:
        ...
        try:
            ...
            print(f"❌ Failed to preprocess {image_type} image: {e}")
            raise
    ...
    async def analyze_with_llm(question_image_path: str, working_note_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            question_image_b64 = await MathEvaluationActivities._encode_image_to_base64(question_image_path)
            working_note_b64 = await MathEvaluationActivities._encode_image_to_base64(working_note_path)
            ...
            if settings.openai_api_key:
                try:
                    analysis_result = await MathEvaluationActivities._analyze_with_openai(
                        question_image_b64, working_note_b64
                    )
                except Exception as e:
            ...
            if not analysis_result and settings.azure_openai_api_key:
                try:
                    analysis_result = await MathEvaluationActivities._analyze_with_azure_openai(
                        question_image_b64, working_note_b64
                    )
                except Exception as e:
            ...
            if not analysis_result and settings.anthropic_api_key:
                try:
                    analysis_result = await MathEvaluationActivities._analyze_with_anthropic(
                        question_image_b64, working_note_b64
                    )
                except Exception as e:
            ...
            print(f"❌ Failed to analyze with LLM: {e}")
            raise
    ...
    async def save_evaluation_result(
        ...
        try:
            ...
            # Cache in Redis
            await MathEvaluationActivities._cache_evaluation_result(evaluation_log)
